...
function getGmailClient() {
  console.log(`[Email] Credenciales: CLIENT_ID=${GMAIL_CLIENT_ID ? "OK" : "VAC├ìO"}, SECRET=${GMAIL_CLIENT_SECRET ? "OK" : "VAC├ìO"}, REFRESH=${GMAIL_REFRESH_TOKEN ? "OK" : "VAC├ìO"}, USER=${EMAIL_USER || "VAC├ìO"}`);
...
export async function sendPurchaseConfirmation(params: {
  ...
  const gmail = getGmailClient();
  if (!gmail) {
    console.log("[Email] No se puede enviar ÔÇö email no configurado");
    return false;
  }
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:20px;">
        ...
        <div style="background:#f0f8f0;padding:15px;border-radius:8px;margin:15px 0;">
          <h3 style="margin:0 0 10px;color:#1a7a2e;">­ƒôª Direccion de entrega:</h3>
  ...
  try {
    ...
    try {
      ...
      console.error("[Email] Error enviando notificaci├│n a vendedora:", e);
    }
...
async function sendSellerNotification(params: {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#ff9800;color:white;padding:20px;text-align:center;">
        <h1 style="margin:0;">Nueva Venta! ­ƒÄë</h1>
      </div>
      <div style="padding:20px;">
        ...
        <div style="background:#fff3e0;padding:15px;border-radius:8px;margin:15px 0;">
          <p style="margin:0;"><strong>­ƒôª Direccion de entrega:</strong></p>
  ...
  const subject = `­ƒÆ░ Nueva venta #${params.orderId.substring(0, 8)} - ${params.buyerName} - $${totalFormatted}`;
...
export async function sendReservationConfirmation(params: {
  ...
  try {
    await sendEmail(
      params.to,
      `Reserva #${params.orderId.substring(0, 8)} - Pago contra entrega`,
      html
    );
